        ...
        const struct light_state_t *state)
{
    ...
    pthread_mutex_lock(&g_lock);
    err = write_int(RED_LED_FILE, brightness);
    pthread_mutex_unlock(&g_lock);
    return err;
}
...
{
    int brightness = rgb_to_brightness(state);
    int blink, onMS, offMS, stepDuration, pauseHi;
    char *duty;
    ...
    ALOGV("set_speaker_light_locked mode %d, brightness=%08X, onMS=%d, offMS=%d\n",
            state->flashMode, brightness, onMS, offMS);
    ...
    if (blink) {
        ...
        // Start the party
        write_int(RED_BLINK_FILE, brightness);
    } else {
